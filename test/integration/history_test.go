...
// echo "markdown,history" > /.build/memory → creates table + view + history table + trigger
func TestSynth_HistoryBuildWithHistory(t *testing.T) {
	...
	fsErr := ops.WriteFile(ctx, "/.build/memory", []byte("markdown,history\n"))
	require.Nil(t, fsErr, "WriteFile to .build/memory should succeed: %v", fsErr)

	// Root should list both _memory (backing table) and memory (synth view)
	entries, fsErr := ops.ReadDir(ctx, "/")
	require.Nil(t, fsErr, "ReadDir root should succeed")

	names := fsEntryNames(entries)
	assert.Contains(t, names, "_memory", "root should contain backing table _memory")
	assert.Contains(t, names, "memory", "root should contain view memory")

	// Verify history table exists via native access
	assert.Contains(t, names, "_memory_history", "root should contain history table")

	// The memory synth view should show .history/ in directory listing (even when empty)
	entries, fsErr = ops.ReadDir(ctx, "/memory")
	require.Nil(t, fsErr, "ReadDir /memory should succeed")
	names = fsEntryNames(entries)
	assert.Contains(t, names, ".history", "memory view should contain .history")

	// Stat .history/ should succeed
	entry, fsErr := ops.Stat(ctx, "/memory/.history")
	require.Nil(t, fsErr, "Stat /memory/.history should succeed")
	assert.True(t, entry.IsDir, ".history should be a directory")
}

// TestSynth_HistoryAddLater tests adding history to an existing app.
// First create a markdown app, then echo "history" > /.build/memory
func TestSynth_HistoryAddLater(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/.build/memory", []byte("history\n"))
	require.Nil(t, fsErr, "WriteFile history should succeed: %v", fsErr)

	// Verify .history/ now appears
	entries, fsErr = ops.ReadDir(ctx, "/memory")
	require.Nil(t, fsErr, "ReadDir /memory should succeed after adding history")
	names = fsEntryNames(entries)
...
func TestSynth_HistoryCaptureUpdate(t *testing.T) {
	...
	// Create a file
	v1Content := "---\ntitle: Hello\nauthor: alice\n---\n\nFirst version.\n"
	fsErr = ops.WriteFile(ctx, "/memory/hello.md", []byte(v1Content))
	...
	v2Content := "---\ntitle: Hello\nauthor: alice\n---\n\nSecond version.\n"
	fsErr = ops.WriteFile(ctx, "/memory/hello.md", []byte(v2Content))
	require.Nil(t, fsErr, "WriteFile v2 should succeed")

	// Current file should show v2
	content, fsErr := ops.ReadFile(ctx, "/memory/hello.md")
	...
	entries, fsErr := ops.ReadDir(ctx, "/memory/.history/hello.md")
	...
	histContent, fsErr := ops.ReadFile(ctx, "/memory/.history/hello.md/"+versionID)
...
func TestSynth_HistoryCaptureDelete(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/memory/doomed.md", []byte(content))
	...
	versions, fsErr := ops.ReadDir(ctx, "/memory/.history/doomed.md")
	...
	histContent, fsErr := ops.ReadFile(ctx, "/memory/.history/doomed.md/"+versionID)
...
func TestSynth_HistoryMultipleVersions(t *testing.T) {
	...
	// Create initial file
	fsErr = ops.WriteFile(ctx, "/memory/evolving.md",
		[]byte("---\ntitle: Evolving\n---\n\nVersion 1.\n"))
	require.Nil(t, fsErr, "WriteFile v1 should succeed")
	...
	for i := 2; i <= 4; i++ {
		...
		fsErr = ops.WriteFile(ctx, "/memory/evolving.md", []byte(body))
		require.Nil(t, fsErr, "WriteFile v%d should succeed", i)
	}

	// Should have 3 history entries (v1, v2, v3 — v4 is current)
	entries, fsErr := ops.ReadDir(ctx, "/memory/.history/evolving.md")
...
func TestSynth_HistoryReadOnly(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/memory/.history/fake.md", []byte("should fail"))
	...
	fsErr = ops.Mkdir(ctx, "/memory/.history/fake-dir")
...
func TestSynth_HistoryIDFile(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/memory/test.md",
		[]byte("---\ntitle: Test\n---\n\nOriginal.\n"))
	require.Nil(t, fsErr, "WriteFile v1 should succeed")
	...
	// Read .id file
	content, fsErr := ops.ReadFile(ctx, "/memory/.history/test.md/.id")
...
func TestSynth_HistoryByUUID(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/memory/tracked.md",
		[]byte("---\ntitle: Tracked\n---\n\nOriginal content.\n"))
	require.Nil(t, fsErr, "WriteFile v1 should succeed")
	...
	idContent, fsErr := ops.ReadFile(ctx, "/memory/.history/tracked.md/.id")
	require.Nil(t, fsErr, "ReadFile .id should succeed")
	rowUUID := strings.TrimSpace(string(idContent.Data))

	// List .by/ should contain this UUID
	entries, fsErr := ops.ReadDir(ctx, "/memory/.history/.by")
	...
	entries, fsErr = ops.ReadDir(ctx, "/memory/.history/.by/"+rowUUID)
	...
	content, fsErr := ops.ReadFile(ctx, "/memory/.history/.by/"+rowUUID+"/"+versionID)
...
func TestSynth_HistoryDiff(t *testing.T) {
	...
	fsErr = ops.WriteFile(ctx, "/memory/doc.md",
		[]byte("---\ntitle: Document\nauthor: alice\n---\n\nFirst draft.\n"))
	require.Nil(t, fsErr, "WriteFile v1 should succeed")
	...
	current, fsErr := ops.ReadFile(ctx, "/memory/doc.md")
	require.Nil(t, fsErr, "ReadFile current should succeed")
	assert.Contains(t, string(current.Data), "Revised draft.")

	// Read historical version
	entries, fsErr := ops.ReadDir(ctx, "/memory/.history/doc.md")
	require.Nil(t, fsErr, "ReadDir history should succeed")
	...
	historical, fsErr := ops.ReadFile(ctx, "/memory/.history/doc.md/"+versionID)
